...
------------------------------------------------------------------------------*/    
    ...
    from sirketler sir		                -- fromdan sonraki sol daki tablo
    inner join siparisler sip	            -- inner den sonraki sagdaki tablo oluyor
    ON sir.sirket_id=sip.sirket_id;         -- sirketler.sirket_id=siparisler.sirket_id
											-- where yerine on yazilacak
...
CREATE TABLE bolumler (
 ...
 /* -----------------------------------------------------------------------------
  ...
  -- baglama isi join lerde on keyword ile yapılır
...
------------------------------------------------------------------------------*/  
  ...
  join personel p
  on p.bolum_id=b.bolum_id
  order by b.bolum_isim desc, p.maas;  -- maasları bolumler ıcersınde ayrı saralı yapıyor
...
------------------------------------------------------------------------------*/ 
  ...
  maas siralı olarak  listeleyiniz.
------------------------------------------------------------------------------*/   
  
  select b.bolum_isim, p.personel_isim, p.maas
  from bolumler b 
  right join personel p
  on p.bolum_id=b.bolum_id
  where p.mudur_id in(1222,1333);
  
  /* -----------------------------------------------------------------------------
  ORNEK6: MUDUR'u Harun veya Emine olan personelin bolumlerini,isimlerini,  
  maaslarini ve ayrica MUDUR isimlerini 
