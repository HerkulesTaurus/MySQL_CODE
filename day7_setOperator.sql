...
SELECT isim as isim_sehir, maas -- alttire_ ile isimlendirme snake 
FROM personel					-- kebapcase - ortada tire
where maas>4000					-- buyuk kucuk camelcase
union
select sehir, maas
from personel 
where maas>5000;
/* -----------------------------------------------------------------------------
  ...
  sorguyu yaziniz.    
------------------------------------------------------------------------------*/ 
select maas, isim as  isim_sehir -- as ve sonrası tabloya yenı ısım verme 
from personel
...
/* -----------------------------------------------------------------------------
  ORNEK5: Ismi Mehmet Ozturk olanlarin sehirlerini + olarak Istanbul'da calismayanlarin 
...
/*======================== FARKLI TABLOLARDAN BIRLESTIRME ====================*/   
    ...
    /* -----------------------------------------------------------------------------
      ...
      ------------------------------------------------------------------------------*/ 
select sehir as sehir_tel, maas as maas_cocukSayisi -- yenıden adlandırdık as ıle 
from personel
where id=123456789
union
...
-- sirketlerden grupla sirketlerin calısan isimlerini say
