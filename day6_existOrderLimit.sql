...
WHERE EXISTS (SELECT urun_id FROM nisan WHERE mart.urun_id=nisan.urun_id);
						-- urun_id ayni oldugu icin basına mart. ve nisan. yazdık 
/*-----------------------------------------------------------------------------
  ORNEK2: Her iki ayda birden satılan ürünlerin URUN_ISIM'lerini ve bu ürünleri
  NİSAN ayında satın alan MUSTERI_ISIM'lerini listeleyen bir sorgu yazınız. 
  -----------------------------------------------------------------------------*/
  
  SELECT urun_isim, musteri_isim FROM nisan as n   -- urun_id basına nisan. yazmamak icin as n yazdık 
  WHERE EXISTS(SELECT urun_isim FROM mart m WHERE n.urun_isim=m.urun_isim); -- alta da marttan sonra m
  
  /*-----------------------------------------------------------------------------
  ORNEK3: Her iki ayda ortak satılmayan ürünlerin URUN_ISIM'lerini ve bu ürünleri
  NİSAN ayında satın alan MUSTERI_ISIM'lerini listeleyen bir sorgu yazınız. 
  -----------------------------------------------------------------------------*/
  ...
  /*===================== IS NULL, IS NOT NULL, COALESCE ========================
    ...
    Ornek1; isim i null olanlari sorgula
     ...
     /*====================================================================
    Ornek2; isim i null olmayanlari sorgula
      ====================================================================*/
...
-- tabloyu eski haline dondur null lar NO NAME yaptık
...
/* ----------------------------------------------------------------------------
 ...
 -----------------------------------------------------------------------------*/
      ...
      order by adres;
/* ----------------------------------------------------------------------------
  ORNEK2: kisiler tablosunu maas'e göre sıralayarak sorgulayınız.
 -----------------------------------------------------------------------------*/
 
 select * from kisiler 
      order by maas;
      
-- maas a gore ters sirala
...
/*----------------------------------------------------------------------------
 ...
 select * from kisiler 
 where soyisim ='Bulut'
 order by 5; -- sutun sayisini yazarsak ona gore sirlaar ismini yazmasak da olur 
...
select * from kisiler limit 10,2; -- ilk sayi dahil degil 11 den basla, 2. say git(2 tane git al)
-- 2. yol 
